...
export const CalendarView: React.FC = () => {
  ...
  const [calendarFilter, setCalendarFilter] = useState<'all' | EntryType>('all');
  ...
  // Group entries by date ISO key, applying type filter
  const entriesByDate = entries.reduce<Record<string, Entry[]>>((acc, entry) => {
    if (calendarFilter !== 'all' && entry.type !== calendarFilter) {
      return acc;
    }
  ...
  return (
    <div className="w-full max-w-4xl mx-auto px-6 flex flex-col h-full overflow-y-auto no-scrollbar">
      ...
      <div className="sticky top-0 z-30 bg-slate-950/85 backdrop-blur-xl border-b border-white/10 -mx-6 px-6 pt-3 pb-4 mb-4 flex items-center justify-between flex-shrink-0">
        ...
        <div className="flex items-center gap-2">
          ...
          <select
            ...
            onChange={(e) => setCalendarFilter(e.target.value as 'all' | EntryType)}
            className="bg-slate-900/90 text-xs font-semibold text-slate-200 border border-white/20 rounded-xl px-3 py-1.5 focus:outline-none focus:border-orange-400 cursor-pointer shadow-sm"
          >
      ...
      {/* Calendar Month Navigation Header & Grid */}
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center justify-center gap-1 h-2">
                    {hasNote && (calendarFilter === 'all' || calendarFilter === 'note') && (
                      <span className="w-1.5 h-1.5 rounded-full bg-emerald-400" title="Note" />
                    )}
                    {hasList && (calendarFilter === 'all' || calendarFilter === 'list') && (
                      <span className="w-1.5 h-1.5 rounded-full bg-amber-400" title="List" />
                    )}
                    {hasVocab && (calendarFilter === 'all' || calendarFilter === 'vocab') && (
                      <span className="w-1.5 h-1.5 rounded-full bg-indigo-400" title="Vocab" />
                    )}
                  </div>
                </button>
              );
          ...
          <div className="bg-slate-900/80 backdrop-blur-md rounded-2xl p-6 border border-white/15 shadow-card flex flex-col gap-4">
            ...
            {selectedDayEntries.length === 0 ? (
              <div className="py-8 text-center text-sm text-slate-400 italic">
                No matching entries recorded on this day.
              </div>
            ) : (
